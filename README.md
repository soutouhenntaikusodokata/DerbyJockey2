...
このREADMEは **DerbyJockey Modern 1.4.6-klnefak** の実装を基準にしています。
1.4.5はスタミナの回復力が早すぎてお勧めできない（）
## 目次
...
4. `DerbyJockey-Modern-1.4.6-klnefak.jar`を`plugins`へ入れます。
...
標準設定は30tickごとに最大スタミナの1%です。20TPSなら1.5秒ごと、毎秒約0.67%です。
これはv1.4.5までの標準値（5tickごとに1%）と比べて6倍遅い回復です。
最大値を超えた分は切り捨てられます。
...
stamina:
  ...
  recovery-interval-ticks: 30
  recovery-percent-per-interval: 0.01
  exhaustion-minimum-duration-seconds: 25
  exhaustion-maximum-duration-seconds: 45
...
| `stamina.recovery-interval-ticks` | `5` | `30` | v1.4.6で本気度0の回復を6倍遅くする |
| `stamina.exhaustion-minimum-duration-seconds` | `60` | `25` | v1.4.6で疲労抽選の下限を変更する（最大値と同時移行） |
| `stamina.exhaustion-maximum-duration-seconds` | `120` | `45` | v1.4.6で疲労抽選の上限を変更する（最小値と同時移行） |

これらと完全に一致しない手動設定値は維持します。疲労時間は最小値と最大値が
両方とも旧標準値に一致する場合だけ移行します。旧バージョンで使用していた不要なキーは
自動削除しません。旧標準値を意図的に使いたい場合は、初回移行後にもう一度設定してください。
...
| `recovery-interval-ticks` | `30` | 本気度0の回復間隔。最低1tick |
| `recovery-percent-per-interval` | `0.01` | 1回あたり最大スタミナ回復率 |
| `exhaustion-minimum-duration-seconds` | `25` | 疲労時間の抽選下限。最低1秒 |
| `exhaustion-maximum-duration-seconds` | `45` | 疲労時間の抽選上限。下限未満なら下限を使用 |
...
```text
target/DerbyJockey-Modern-1.4.6-klnefak.jar
```
