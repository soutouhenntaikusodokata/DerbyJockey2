...
このREADMEは **DerbyJockey Modern 1.4.2-klnefak** の実装を基準にしています。
...
4. `DerbyJockey-Modern-1.4.2-klnefak.jar`を`plugins`へ入れます。
...
走行中 = 水平速度の二乗 > velocity-threshold-squared
```
...
stamina:
  ...
  whip-cost: 0.035
  velocity-threshold-squared: 0.0004
...
既存の設定値は上書きしません。旧バージョンで使用していた不要なキーは自動削除しません。
...
| `velocity-threshold-squared` | `0.0004` | 水平速度二乗の走行判定閾値。最低0 |
...
```text
target/DerbyJockey-Modern-1.4.2-klnefak.jar
```
