...
このREADMEは **DerbyJockey Modern 1.4.3-klnefak** の実装を基準にしています。
...
4. `DerbyJockey-Modern-1.4.3-klnefak.jar`を`plugins`へ入れます。
...
最大スタミナは、未改変版DerbyJockeyが使用していたUUID由来の消費補正を、
実効スタミナ容量へ換算した値です。
...
|---|---:|---:|
| 未改変版補正 | 0.55 | 1.00 |
| 最大スタミナ | 100.00 | 約181.82 |

実装ではUUIDのhash値を数値化し、9桁未満の場合は馬の最大体力を利用して拡張し、
9桁へパディングした後の末尾5桁を計算に使います。

最大スタミナ、補正モデル番号、互換用endurance、基礎移動速度は馬のPDCへ保存されます。
旧Modern版の独自スタミナ値は、モデル番号が古いか存在しない場合、次にその馬を読み込んだ時点で
この方式へ一度だけ再計算されます。

## スタミナの走行判定

プレイヤーの速度や座標差ではなく、騎乗中の馬自身の`getVelocity()`を毎tick確認します。

```text
水平速度の二乗 = velocityX² + velocityZ²
速度検出 = 水平速度の二乗 > velocity-threshold-squared
走行中 = 現在速度を検出、または最後の速度検出から猶予tick以内
```

標準の閾値は`0.000001`、速度検出猶予は5tickです。
Paperが騎乗馬の速度を一時的に0として返すtickが混ざっても、最後に有効な馬速度を検出してから
5tick（20TPSなら0.25秒）は走行状態を維持し、消費処理が途切れないようにしています。
...
テレポートは速度ベクトルを伴わない限り、座標差だけでは走行扱いになりません。
...
スタミナ品質 = normalize(最大スタミナ, 100, 181.82)
総合品質 = (本気度品質 + スタミナ品質) / 2
...
- 本気度に対応する消費倍率
- 走行中に現在適用している毎秒消費量
- 本気度1以上で馬速度を検出できていない場合の`速度未検出`表示
- 覚醒中の残り秒数
- 疲労困憊中の残り秒数
...
診断の`消費倍率`が3.25でも`現在適用`が0の場合は、倍率ではなく速度判定が無効です。
...
| `derbyjockey:endurance` | DOUBLE | 旧版互換用の最大スタミナ÷100 |
...
既存の設定値は原則として上書きしません。ただし、v1.4.2以前の標準速度閾値
`stamina.velocity-threshold-squared: 0.0004`がそのまま残っている場合だけ、
走行を検出しやすい新標準値`0.000001`へ自動移行します。
標準値以外へ手動変更した設定は維持します。旧バージョンで使用していた不要なキーは自動削除しません。
...
- 水流、ノックバック、横滑りも水平速度が閾値を超えれば走行扱いです。
- 速度検出が一時的に0になっても、標準では最後の検出から5tickだけ走行扱いを維持します。
...
- 本気度0では継続消費しません。
- 本気度1以上でも水平速度が閾値以下なら消費しません。
- `stamina.drain-per-second`が0になっていないか確認します。
- `stamina.velocity-threshold-squared`が大きすぎないか確認します。
- 馬へ乗ったまま`/derbyhorse debug`を実行し、水平速度、走行判定、倍率、現在適用量を確認します。
- ボスバーが`速度未検出`なら倍率計算前の走行条件で止まっています。
...
```text
target/DerbyJockey-Modern-1.4.3-klnefak.jar
```
